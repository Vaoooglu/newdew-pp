...
- `Автор рецептов API` — обязательный пользователь WordPress, от имени которого создаются записи.
...
Облачный Telegram Bot API позволяет боту скачать файл размером не более 20 МБ. Дополнительно применяется лимит загрузки WordPress, если он меньше. Бот проверяет размер сразу при получении файла и предлагает уменьшить видео, не переводя мастер к подтверждению.

Перед началом диалога и повторно перед созданием записи бот проверяет выбранного автора и необходимые права. Если автор не настроен, бот сразу подсказывает раздел настроек вместо ошибки в конце мастера.
